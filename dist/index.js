...
var RULES_DIR = join(dirname(fileURLToPath(import.meta.url)), "..", "..", "rules");
var RuleLoader = class {
  ...
  load() {
    ...
    return readdirSync(RULES_DIR).filter((f) => f.endsWith(".json")).map((f) => JSON.parse(readFileSync(join(RULES_DIR, f), "utf-8")));
  }
};
